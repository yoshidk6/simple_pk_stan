...
# Summary
Multiple ideas worked to avoid label switching, (1) weak prior on all parameters, (2) limit of Vd lower value (with weak priors), (3) ordered variables for KA and KEL, (4) limit ka with CL and VD + provide weak priors and initial values.  
(4) to be picked for consistency with population model. 

The model in the blog is as follows:
...
init <- function(){
    list(CL = exp(rnorm(1, log(0.5), 0.2)),
...
fit.stan <-
  stan(file = "mod_00_single_subj_lim_ka_eq.stan", 
       data = data,
       init = init)
